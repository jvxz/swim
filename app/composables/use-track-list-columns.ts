...
export const useTrackListColumns = createSharedComposable(() => {
  const { layoutPanels } = usePersistentPanels(
    'track-list-columns',
    [
      3, 1.5, 34.0888869203, 15.1599593363, 15.0211805522, 8.5755556775, 14.3210841804,
      8.3333333333,
    ],
  )

  const { createWindow: openSetDisplayedFieldsWindow, window: setDisplayedFieldsWindow } =
    useTauriWindow('track-list-columns', {
      height: 900,
  ...
  function getColumnFields(
    as = 'objects' as 'keys' | 'objects',
  ): WritableComputedRef<TrackListColumn['key'][]> | WritableComputedRef<TrackListColumn[]> {
    return as === 'keys'
      ? computed<TrackListColumn['key'][]>({
          get: () => settings.layout.element.trackList.columnFields,
          set: (value: TrackListColumn['key'][]) =>
            (settings.layout.element.trackList.columnFields = value),
        })
      : computed<TrackListColumn[]>({
          get: () =>
            settings.layout.element.trackList.columnFields.map((key) => getColumnFromKey(key)),
          set: (value: TrackListColumn[]) =>
            (settings.layout.element.trackList.columnFields = value.map((column) => column.key)),
        })
  }

  watch(
    () => settings.layout.element.trackList.columnFields,
    (newColumns, oldColumns) => {
      ...
      if (newColumns.length > oldColumns.length) {
        ...
        const newLayoutPanelSizes = layoutPanels.value.map((p, i) =>
          i === newColIdx ? newColSize : p - toSubtract,
        )
  ...
  function getColumnFromKey(key: TrackListColumn['key']): TrackListColumn {
    ...
    if (!col)
      throw new Error(
        `Attempted to get column from key ${key} but it was not found in the list of all columns`,
      )
